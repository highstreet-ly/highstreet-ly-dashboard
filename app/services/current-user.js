...
export default Service.extend({
    ...
    async getStats() {
        let stats = await this.eventOrganiser.dashboardStats;

        await this.statsService.initData(stats);
    },
    ...
    async loadInternal() {
        ...
        try {
            if (this.session.isAuthenticated) {
                ...
                let userModel = await this.store.findRecord('user', user.sub, { include: 'roles,claims' });

                this.set('userModel', userModel);

                if (userModel.currentEoid) {
                    let organiser = await this.store.findRecord('event-organiser', userModel.currentEoid);

                    let subs = await this.store.query('subscription', {
                        filter: `expr:and(equals(event-organiser-id,'${userModel.currentEoid}'),or(equals(status,'in_trial'),equals(status,'active')))`
                    })

                    this.set('currentSubscription', subs.firstObject)
    ...
    isEventOrganiser: computed('user.role.[]', function () {
        return this.isInRoleSync('EventOrganiser')
    }),

    isDashUser: computed('user.role.[]', function () {
        return this.isInRoleSync('DashUser')
    }),


    isOperator: computed('user.role.[]', function () {
    ...
    hasFeatureSync(feature) {

        function findFeature(element) {
        ...
        console.log(this.userModel.feature)

        if (this.user.feature && this.user.feature.find && this.user.feature.find(findFeature)) {
            return true;
        }

        // this is dumb but if there is only one element in the roles array
        // it shows up as a string not a one element array
        if (this.user.feature && this.user.feature === feature) {
            return true;
        }
